...
def _merge_callbacks(
    ...
    """Merge user callbacks with instrumentation callback.

    Extracts callbacks from both config["callbacks"] and kwargs["callbacks"],
    combines them with our tracer, and updates kwargs.

    Args:
        kwargs: Invoke/ainvoke kwargs dictionary (will be mutated)
        callback_tracer: LangGraphTracer instance to inject

    Returns:
        Updated kwargs with merged callbacks in config
    """
...
class LangGraphInstrumentor(UiPathInstrumentor):
    ...
    def _instrument(self, **kwargs: Any) -> None:
        ...
        # Try to get telemetry_client, but fall back to tracer_provider if client not initialized
        try:
            ...
            if tracer_provider is None:
                raise RuntimeError(
                    "Telemetry client not initialized and no tracer_provider provided. "
                    "Either call telemetry.init() first or pass tracer_provider to instrument()."
                ) from e
            ...
            class _TestClientWrapper:
                ...
                def __init__(self, provider: Any) -> None:
                    ...
                    self._tracer = get_tracer(
                        instrumenting_module_name="uipath.core.telemetry.integrations.langgraph",
            ...
            telemetry_client = _TestClientWrapper(tracer_provider)  # type: ignore[assignment]
        ...
        def traced_compile(
            ...
            **compile_kwargs: Any,
            ...
            def traced_invoke(*args: Any, **kwargs: Any) -> Any:
                ...
                tracer = telemetry_client.get_tracer()
                with tracer.start_as_current_span("langgraph.invoke") as span:
                    ...
                    # Create tracer and merge callbacks
                    callback_tracer = LangGraphTracer(
                        telemetry_client=telemetry_client,
                        trace_state=True,
                        trace_edges=True,
                    )
                    kwargs = _merge_callbacks(kwargs, callback_tracer)
