...
from uipath.core.telemetry.integrations_full.langgraph import LangGraphInstrumentor
    ...
    resource = Resource.create(
        {
            ...
            "telemetry.sdk.name": "uipath-telemetry",
        }
    )
...
@pytest.fixture
def telemetry_client(tracer_provider_and_exporter):
    """Initialize UiPath Telemetry client using pre-configured provider."""
    from uipath.core.telemetry import client as client_module
    ...
    test_client._tracer = provider.get_tracer("uipath.core.telemetry")
...
@pytest.fixture
def instrumentor(telemetry_client):
...
def test_basic_sync_execution(instrumentor, simple_workflow, exporter, telemetry_client):
...
@pytest.mark.no_auto_tracer
@pytest.mark.asyncio
async def test_async_execution(instrumentor, simple_workflow, exporter, telemetry_client):
...
@pytest.mark.no_auto_tracer
def test_error_handling(instrumentor, exporter, telemetry_client):
...
@pytest.mark.no_auto_tracer
def test_large_state_truncation(instrumentor, exporter, telemetry_client):
...
@pytest.mark.no_auto_tracer
def test_reinstrumentation_guard(telemetry_client):
...
@pytest.mark.no_auto_tracer
def test_callback_preservation(instrumentor, simple_workflow, exporter, telemetry_client):
...
@pytest.mark.no_auto_tracer
def test_uninstrumentation(telemetry_client, exporter):
