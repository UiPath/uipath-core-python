...
def test_generation_observation_update(in_memory_exporter: InMemorySpanExporter) -> None:
    ...
    # Execute
    with Trace(tracer, "root") as trace_ctx:
        with trace_ctx.generation("llm-call") as obs:
    ...
    assert len(gen_span.attributes) > 1  # More than just span.type
...
def test_observation_exit_on_exception(in_memory_exporter: InMemorySpanExporter) -> None:
    ...
    with pytest.raises(ValueError, match="test error"):
        with Trace(tracer, "root") as trace_ctx:
            with trace_ctx.generation("test"):
...
def test_agent_observation_specific(in_memory_exporter: InMemorySpanExporter) -> None:
    ...
    # Execute
    with Trace(tracer, "root") as trace_ctx:
        with trace_ctx.agent("reasoning-agent") as obs:
    ...
    assert agent_span.attributes.get("agent.name") == "reasoning-agent"


def test_embedding_observation_with_model(
    in_memory_exporter: InMemorySpanExporter,
) -> None:
    """Test EmbeddingObservation sets model attribute.
    ...
    # Execute
    with Trace(tracer, "root") as trace_ctx:
        with trace_ctx.embedding("embed", model="text-embedding-3-small") as obs:
    ...
    assert embed_span.attributes.get("gen_ai.request.model") == "text-embedding-3-small"
