...
from .._shared import TTLSpanRegistry
...
class LangGraphTracer(BaseCallbackHandler):
    ...
    Attributes:
        otel_client: UiPath OTel client instance
    ...
    def __init__(
        ...
        # CRITICAL FIX: Use TTL registry to prevent memory leaks from orphaned spans
        # TTL of 1 hour ensures spans are cleaned up even if on_chain_end never called
        self._spans: TTLSpanRegistry[UUID, Span] = TTLSpanRegistry(ttl_seconds=3600)
        # CRITICAL FIX: Store context tokens for proper span activation/deactivation
        self._tokens: TTLSpanRegistry[UUID, object] = TTLSpanRegistry(ttl_seconds=3600)
    ...
    def on_chain_start(
        ...
        **kwargs: Any,
        ...
        tracer = self.otel_client.get_tracer()
        ...
        from uipath.core.otel.integrations._shared import get_session_id, get_thread_id
    ...
    def _filter_state(
        ...
        """
        try:
            # Custom JSON encoder to handle common LangChain objects
            ...
            serialized = json.dumps(state, default=_serialize_default)
